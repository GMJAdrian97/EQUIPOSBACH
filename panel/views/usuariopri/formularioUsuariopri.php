<div class="contenedor">
    <!-- <img src="../../../Imagenes/Fondo2.jpg" alt="ImagenesF" /> -->
    <div class="centradoUsuario">
        <div class="row">
            <div class="col">
                <!-- multistep form -->
                <form method="POST"
                    action="ctrlUsuariopri.php?accion=<?php echo(isset($id_usuariopri))? "update&id_usuariopri=".$id_usuariopri: "add"; ?>"
                    enctype="multipart/form-data" id="msform">
                    <!-- fieldsets -->
                    <fieldset id="fieldset">
                        <h2 class="fs-title">
                            <?php echo(isset($id_usuariopri))? "Modifica a tu ": " Introduce tu nevo ";?>Usuario privilegiado</h2>
                        <br />
                        <div class="col-auto">
                            <label class="sr-only" for="inlineFormInputGroup">Correo</label>
                            <div class="input-group mb-2">
                                <div class="input-group-prepend">
                                    <div class="input-group-text"><i class="bi bi-journal-plus"></i></div>
                                </div>
                                <input type="text" name="correo"
                                    value="<?php echo(isset($id_usuariopri)) ? $datosUsuariopri['correo']:"";?>"
                                    class="form-control" id="inlineFormInputGroup" placeholder="correo"
                                />
                                <label class="sr-only" for="inlineFormInputGroup">Pass</label>
                                <input type="text" name="pass"
                                    value="<?php echo(isset($id_usuariopri)) ? $datosUsuariopri['pass']:"";?>"
                                    class="form-control" id="inlineFormInputGroup" placeholder="pass"
                                />
                            </div>
                        </div>
                        <br />
                        <br />
                        <input class="btn btn-primary" type="submit" value="Guardar" />
                    </fieldset>
                </form>
            </div>
        </div>
    </div>
</div>